    ...
    for embedding_bytes_lazy in embeddings_bytes_lazy:
        ...
        for knn_embedding in get_file_repository().get_embedding_generator_by_knn(
            ...
            if found_file and similarity > settings.auto_tag_file_similarity_threshold:
                files.append(FileScore(file=found_file, score=similarity))

    return files


@app.task(base=FileIndexingTask)
def calculate_tag_score(files: list[FileScore], file: File) -> list[TagScore]:
    all_tags: list[TagScore] = [
        TagScore(tag=tag, score=file.score) for file in files for tag in file.file.tags
    ]
    ...
    valid_tags: list[TagScore] = [t for t in tags_avg_score if t.tag not in file.tags]
