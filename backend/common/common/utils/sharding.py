...
def compute_shard(entity_id: UUID, num_shards: int) -> int:
    ...
    """
    hash_bytes = hashlib.sha256(entity_id.bytes).digest()
    return int.from_bytes(hash_bytes, byteorder="big") % num_shards


def get_persister_shard_name(shard: int) -> str:
    ...
    """
    return f"{settings.celery_persister_shard_prefix}.{shard}"


def get_all_persister_shards(num_shards: int) -> list[str]:
    """Get all persister shard names.

    Args:
        num_shards: The total number of shards.

    Returns:
        List of all persister shard names.
    """
    return [get_persister_shard_name(i) for i in range(num_shards)]
...
def get_persister_shard_for_worker(
    persister_id: int,
    persister_total: int,
    num_shards: int,
) -> list[str]:
    """Get the names for shards assigned to this persister worker.
    ...
    Returns:
        List ofnames for the shards assigned to this worker.
    """
    shards = get_shards_for_persister(persister_id, persister_total, num_shards)
    return [get_persister_shard_name(s) for s in shards]
