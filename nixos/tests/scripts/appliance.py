...
The point of this test is drift: box.nix restates values that live in up.sh and vars.sh.
If someone changes a sysctl in up.sh `setup_system`, adds a host to vars.sh, or adds a
`check_command` to `validate_environment` without updating the appliance, the box
silently ships misconfigured. This catches that.

Everything the .nix file knows and this one cannot is passed in as `Params`, which is
also the list of things the appliance and up.sh have to agree about.
"""
...
def _sysctls(appliance: "Machine", subtest: "Subtest") -> None:
...
def _host_resolution(appliance: "Machine", subtest: "Subtest", params: Params) -> None:
...
def _hosts_file(appliance: "Machine", subtest: "Subtest") -> None:
...
def _unit_path(appliance: "Machine", subtest: "Subtest", params: Params) -> None:
...
def _yq_flavour(appliance: "Machine", subtest: "Subtest") -> None:
...
def _access_policy(appliance: "Machine", subtest: "Subtest", params: Params) -> None:
...
def _console_font(appliance: "Machine", subtest: "Subtest") -> None:
...
def _console_session(
    ...
    with subtest("a keypress opens the operator's console session"):
        ...
        assert len(panes) == (3 if params.ai_enabled else 2), panes
...
def _pane_restart(
    appliance: "Machine", subtest: "Subtest", params: Params, panes: list[str]
) -> None:
    with subtest("a pane whose program exits gets it back"):
        ...
        dead = appliance.succeed(
            "tmux -S /run/loom/tmux.sock list-panes -t loom -F '#{pane_dead}'"
        ).split()
        assert dead == ["0"] * len(dead), dead

        _log_pane_hands_over(appliance, params, panes)
        ...
        if params.ai_enabled:
            _assistant_pane(appliance, params, panes)
...
def _assistant_pane(appliance: "Machine", params: Params, panes: list[str]) -> None:
...
def _repository(appliance: "Machine", subtest: "Subtest", params: Params) -> None:
...
def _loom_up_flags(appliance: "Machine", subtest: "Subtest", params: Params) -> None:
...
def _loom_up_on_path(appliance: "Machine", subtest: "Subtest") -> None:
...
def _radios(appliance: "Machine", subtest: "Subtest") -> None:
...
def _mode_sessions(appliance: "Machine", subtest: "Subtest", setup_sys: str) -> None:
...
def _boot_splash(appliance: "Machine", subtest: "Subtest") -> None:
...
def _guard_idle(appliance: "Machine", subtest: "Subtest") -> None:
...
def _guard_arms(appliance: "Machine", subtest: "Subtest", params: Params) -> str:
...
def _guard_action_per_mode(
    appliance: "Machine", subtest: "Subtest", setup_sys: str
) -> None:
...
def _guard_powers_off(appliance: "Machine", subtest: "Subtest", key_loop: str) -> None:
...
def run(
    ...
    _sysctls(appliance, subtest)
    _host_resolution(appliance, subtest, params)
    _hosts_file(appliance, subtest)
    _unit_path(appliance, subtest, params)
    _yq_flavour(appliance, subtest)
    _access_policy(appliance, subtest, params)
    ...
    _guard_idle(appliance, subtest)
    key_loop = _guard_arms(appliance, subtest, params)
    key_loop = _guard_cancels(appliance, subtest, params, key_loop)
    key_loop = _guard_foreign_key(appliance, subtest, params, key_loop)
    _guard_action_per_mode(appliance, subtest, setup_sys)
    _guard_powers_off(appliance, subtest, key_loop)
