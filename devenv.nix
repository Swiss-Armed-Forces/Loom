  ...
  enterShell = ''
    init(){
      (
        ...
        # Local-override values files: values-overwrites.yaml is edited by hand,
        # values-up-flags.yaml is generated by up.sh. Neither may ever be committed.
        local_override_values=(
          charts/values-overwrites.yaml
          charts/values-up-flags.yaml
        )
        if [ "${builtins.toJSON config.env.cicd}" = true ]; then
          # Drop the skip-worktree bit and restore the committed content. Runner
          # build directories are reused across jobs and git refuses to update
          # skip-worktree files on checkout, so generated content (up.sh appends
          # the whole trusted CA bundle to values-up-flags.yaml) would otherwise
          # survive into later jobs and fail the check-values-local-overrides and
          # typos hooks.
          git \
            update-index \
              --no-skip-worktree \
          ...
          # Ignore local changes so that each developer can customize them
